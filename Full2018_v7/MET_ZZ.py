...
for i, event in enumerate(chain_tth):
    ...
    for j in range(nGen):
        ...
        # Si la partícula es un neutrino (nu_e, nu_mu, nu_tau)
    ...
    # CÁLCULO VECTORIAL Y FILTRADO DE EVENTOS

    # Se exigen 4 neutrinos en el caso de los Z para aislar H -> ZZ -> 4nu y 2 en el caso de los top para asegurar el decaimiento dileptónico
    es_canal_4nu = (len(nu_solo_zz_indices) == 4)
    es_top_dileptonico = (len(nu_solo_top_indices) == 2) 
